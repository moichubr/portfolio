...
const Projects = () => {
  ...
  return (
    <AnimationScrollHandler setIsVisible={setIsVisible} component={"projects"}>
      <section
        id="projects"
        className="flex flex-col h-auto max-h-auto w-full max-w-full md:px-24 lg:px-48 xl:px-48 px-4 lg:py-3 xl:py-3 mt-0 pb-14 lg:pb-20 xl:pb-20"
      >
        <div
          className={`${
        ...
        <div className="flex flex-col w-full mt-8 gap-12 ">
          {myProjects.map((project, index) => (
            <a
              ...
              key={index}
              className={`${
                isVisible ? "animate-flip-up animate-duration-[2000ms]" : ""
              } w-full h-auto`}
            >
