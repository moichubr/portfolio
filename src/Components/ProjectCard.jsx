...
const ProjectCard = ({project}) => {
    ...
    return(
        <article className="w-full bg-transparent h-auto min-h-[300px]  max-h-auto lg:grid lg:grid-cols-2 xl:grid xl:grid-cols-2 flex flex-col">
            ...
            <div className="lg:col-span-1 xl:col-span-1 w-full h-auto pl-6 lg:rounded-r-3xl xl:rounded-r-3xl bg-white">
        <h1 className="text-lg poppins-regular text-pink-900 font-semibold mb-2 mt-3  w-[90%] tracking-wide pb-2 border-b border-b-gray-200">
                    {project.name}
                  </h1>

                  <p className="poppins-regular text-gray-700 max-w-[90%]  min-h-[120px] text-sm">{t(`projectsdata.${project.id}.description`)}</p>

                  <h2 className="text-[12px] poppins-regular font-bold tracking-tight text-pink-900 border-b-2 border-pink-800 w-fit pr-2 py-0.5 mt-2 mb-1">
                {t('projects.stack')}  </h2>
                        ...
                        {project.technologies?.map((el, index) => {
                            return (
                                <li key={index} className="bg-pink-950 bg-opacity-30 text-white font-semibold text-sm px-3 py-1 rounded-full flex items-center">{el}</li>
                         )
                        })}
                        </ul>
